...
Connection conn=Mydb.getConn();
    ...
    String sql="select * from cj where id='"+id+"'";
    ResultSet rs=stmt.executeQuery(sql);
    rs.next();
   %>
<center>
   <form action="/MyFirstProject/UpdatecjDo">
  ID：<input type="text" name="id" value=<%=rs.getString("id") %> ><br>
   学号：<input type="text" name="xh" value=<%=rs.getString("xh") %>><br>
   课程号：<input type="text" name="kcdh" value=<%=rs.getString("kcdh") %>><br>
   成绩：<input type="text" name="cj" value=<%=rs.getString("cj") %>><br>
